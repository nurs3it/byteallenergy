...
import {
    ...
    PencilSimple as PenLineIcon,
    DotsNine as PentagonIcon,
    Plus,
    Square as SquareIcon,
    Trash as Trash2Icon,
    ArrowCounterClockwise as Undo2Icon,
    Path as WaypointsIcon,
} from "phosphor-react"
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button
                    ...
                    title="Select layers"
                    className={cn(
                        "absolute top-1 right-1 z-10 border",
                        className
                    )}
                    {...props}>
                    <LayersIcon />
...
function MapZoomControl({ className, ...props }: React.ComponentProps<"div">) {
    ...
    return (
        <ButtonGroup
            ...
            className={cn("absolute top-1 left-1 z-40 h-fit", className)}
            {...props}>
            <Button
                type="button"
                size="icon-sm"
    ...
    return (
        <>
            <Button
                ...
                className={cn(
                    "absolute right-1 bottom-1 z-10 border",
                    className
                )}
                {...props}>
                {isLocating ? (
                    <LoaderCircleIcon className="animate-spin" weight="bold" />
                ) : (
                    <NavigationIcon />
                )}
            </Button>
...
function MapDrawControl({
    ...
    return (
        <MapDrawContext.Provider
            ...
            <ButtonGroup
                orientation="vertical"
                className={cn("absolute bottom-1 left-1 z-10", className)}
                {...props}
            />
        </MapDrawContext.Provider>
    )
...
function useMapDrawHandleIcon() {
    ...
    return L.divIcon({
        iconAnchor: [8, 8],
        html: renderToString(
            <CircleIcon className="fill-primary stroke-primary size-4 transition-transform hover:scale-110" weight="fill" />
        ),
    })
}
